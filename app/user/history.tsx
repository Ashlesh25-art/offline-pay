...
<style>
  ...
  <div class="footer">Generated by Offline Pay &bull; ${new Date().toLocaleString('en-IN')}</div>
      ...
      const dest = `${FileSystem.documentDirectory}voucher_${v.voucherId}.pdf`;
      await FileSystem.copyAsync({ from: uri, to: dest });
      const canShare = await Sharing.isAvailableAsync();
      if (canShare) {
        await Sharing.shareAsync(dest, { mimeType: 'application/pdf', dialogTitle: 'Save or Share Voucher PDF', UTI: 'com.adobe.pdf' });
      } else {
...
const styles = StyleSheet.create({
  ...
  voucherDownloadBtn: { backgroundColor: "#2563eb" },
  voucherShareBtn: { backgroundColor: "#16a34a" },
